...
pthread_barrierattr_t customer_thread_attr;

bool request_res(int n_customer, int request[])
{

	printf("\nAllocation Request from customer number %d\n", n_customer);
	printf("\nCustomer attempting to allocate: ");
...
int main(int argc, char *argv[])
{
    ...
    pthread_barrier_init(&customer_thread, &customer_thread_attr, 1);

	// Initialize pthread array of NUM_CUSTOMERS
	/* WRITE YOUR OWN CODE HERE */
	pthread_t ids[NUM_CUSTOMERS];
    pthread_attr_t attr[NUM_CUSTOMERS];
	...
	for(int i = 0; i < NUM_CUSTOMERS; i++)
	{
		...
		*customer = i;
		...
		// Create pthread
		/* WRITE YOUR OWN CODE HERE */
		pthread_create(&ids[i], &attr[i], customerResourceRequest, customer);
