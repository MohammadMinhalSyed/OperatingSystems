...
int main(int argc, char *argv[])
{
    ...
    for (; 1; time_tick++)
    {
        int np = check_for_new_process(	// List of pending processes and time slice
	&pending_processes, time_tick,

	// Queues which processes can be placed on
	&rt, &hi, &me, &lo/*WRITE YOUR CODE HERE*/);
        printf("Time: %03d > %d new processes have arrived\n", time_tick, np);
        dispatch_cycle(&rt, &hi, &me, &lo, &res_avail, &memory /*WRITE YOUR CODE HERE*/);
    }
