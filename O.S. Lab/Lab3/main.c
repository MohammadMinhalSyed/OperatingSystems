...
void read_puzzle() {
	...
	fp = fopen("puzzle.txt" , "r");/* WRITEE YOUR OWN CODE HERE*/;
...
int main(void) {
	...
	for(int i = 0; i < 3; i++) {
		for(int j = 0; j < 3; j++) {
			...
			if(pthread_create(&thread_id[threadcounter], &attr, &check_grid, grid_helper)/* WRITEE YOUR OWN CODE HERE*/) {
	...
	for(int i = 0; i < NUM_CELLS; i++) {
		...
		//Create the thread and check if failed
		if(pthread_create(&thread_id[threadcounter], &attr, &check_row, row_helper)/* WRITEE YOUR OWN CODE HERE*/) {
	...
	for(int i = 0; i < NUM_CELLS; i++) {
		...
		col_helper = (column_data *) calloc(1, sizeof(column_data))/* WRITEE YOUR OWN CODE HERE*/;

		//Set the index
		col_helper->column = i;

		//Create the thread and check if failed
		if(pthread_create(&thread_id[threadcounter], &attr, &check_column, col_helper)/* WRITE YOUR OWN CODE HERE*/) {
	...
	for(int i = 0; i < NUM_THREADS; i++) {
		...
		if(pthread_join(thread_id[i], res)/* WRITEE YOUR OWN CODE HERE*/) {
